...
unzip """$ProjectFolder$apkGodot""" -d folderApkGodot
...
#Se colors.xml não existir cp themes e colors para build res/values
if [ ! -e  """$MyProjectAndroid"""/res/values/colors.xml ]; then

rm -rf """$MyProjectAndroid"""/res/values/themes.xml
cp -rf Theme/themes.xml  """$MyProjectAndroid"""/res/values/themes.xml
cp -rf Theme/colors.xml  """$MyProjectAndroid"""/res/values/colors.xml

else

echo ""
echo "Modifique themes.xml e colors.xml"
echo "Em $MyProjectAndroid/res/values no projeto"
echo ""
...
#Godot
#Mudar para build o res de platform godot
echo "compile res platform_godot"
aapt2 compile """$MyProjectAndroid"""/platform_godot/res/**/* -o  My_Build/compiled_resources/
...
aapt2 link --proto-format -o temporary.apk -I $androidpath/android.jar --manifest $MyProjectAndroid/AndroidManifest.xml -R  My_Build/compiled_resources/*.flat --auto-add-overlay --java  My_Build/gen
